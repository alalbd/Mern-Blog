...
const AllPostList = () => {
    ...
    const { name, email, userID } = JSON.parse(user);

    // Get All Post By User
    useEffect(() => {

        const FeatchData = async () => {
            const res = await fetch(`http://127.0.0.1:5000/api/v1/post/all/${userID}`, {
                method: "GET",
                headers: {
                        ...
                        <div className="card">
                            <div className="card-header">
                                <div className="d-flex justify-content-between">
                                    ...
                                    <NavLink to={"/user/add/new/post"} className="py-1 px-3 border rounded">
                                        Add New Post
                                    </NavLink>
                                </div>
                            </div>
                                        ...
                                        <tbody>
                                            {userPost?.map((value, key) => (
                                                <tr key={key}>
                                                    ...
                                                    <td><StrLength str={value?.desc} strLen={30} /></td>
                                                    <td>{value?.category?.category_name}</td>
                                                    <td>{moment(value?.createdAt).startOf('day').fromNow()}</td>
                                                    <td>
                                                        <NavLink className="f-14 btn btn-success py-0 px-2 me-2 text-white" to={`/user/edit/${value._id}/post`}>Edit</NavLink>
